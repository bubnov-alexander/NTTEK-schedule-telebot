...
def help_message(message):
    ...
    print(f'Пользователь {message.from_user.username} написал {message.text} в ', (datetime.datetime.now(tz).strftime('%H:%M:%S')))
...
#Проверка на список забаненых пользователей, а так же есть ли они в БД
    if message.chat.id not in BAN:
        if data is None:
            ...
            print(f'Пользователь {message.from_user.username} зарегестрировался! в', (datetime.datetime.now(tz).strftime('%H:%M:%S')))
            ...
            if message.content_type.lower() == 'text':
                ...
                if message_to_bot == '👥преподы👥' or message_to_bot == 'преподы':
                    prepod(bot, message)
                    print(f'Пользователь {message.from_user.username} узнал преподов! В', (datetime.datetime.now(tz).strftime('%H:%M:%S')))
                ...
                #Студенты группы 
                    ...
                    print(f'Пользователь {message.from_user.username} запросил студентов в', (datetime.datetime.now(tz).strftime('%H:%M:%S')))
                
                #Рандом пользователя
                elif message_to_bot == '🔁рандомно выбрать студента🔁':
                    myrandom(bot, message)
                    print(f'Пользователь {message.from_user.username} запросил рандом! В', (datetime.datetime.now(tz).strftime('%H:%M:%S')))

                #Информация о боте 
                elif message_to_bot == '📒о боте📒' or message_to_bot == 'о боте':
                    aboutbot(bot, message)
                    print(f'Пользователь {message.from_user.username} узнал о боте в', (datetime.datetime.now(tz).strftime('%H:%M:%S')))
